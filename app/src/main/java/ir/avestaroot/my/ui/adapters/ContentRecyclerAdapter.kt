    ...
    inner class ViewHolder(private val binding: ItemStorageBinding) :
        ...
        fun bind(position: Int) {
           ...
           // Log.d("myapplog", "bind")
            ...
            val yellow = Color.parseColor("#63FFAD15")

            binding.relativeItems.setOnLongClickListener {
                binding.relativeItems.setSelected(!item.isSelected)
                binding.relativeItems.setBackgroundColor(if (item.isSelected) Color.WHITE else yellow)
                ...
                binding.relativeItems.setOnClickListener {
                    binding.relativeItems.setSelected(!item.isSelected)
                    binding.relativeItems.setBackgroundColor(if (item.isSelected ) yellow  else Color.WHITE)
                }
